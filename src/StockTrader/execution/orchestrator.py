...
from StockTrader.execution.order_iface import DataLoader, OrderBuilder, OrderExecutor, ResultPersister
    ...
    def _augment_dataframe(self, df: pd.DataFrame, results: list[ExecutionResult]) -> pd.DataFrame:
        ...
        for r in results:
            ...
            if idx is None or idx not in df.index:
                n_bad += 1
                logger.warning(
                    f"ExecutionResult has no valid df_idx (idx={idx}); " f"result excluded from persisted df"
                )
                continue
