    ...
    while True:
        ...
        try:
            ...
            #     r.set("execution:latest_marks", marked.to_json(), ex=7200)
                ...
                logger.warning(f"unknown {command_type}")
                r.set(result_key, json.dumps({"error": "command iudk"}), ex=3600)
        except Exception as e:
            logger.error(f"fat fucker {command_id}: {e}")
            r.set(result_key, json.dumps({"error": str(e)}), ex=3600)
