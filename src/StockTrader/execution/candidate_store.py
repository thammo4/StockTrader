...
class CandidateStore:
    ...
    def read_candidates(self, mart: str, fname_date: str) -> pd.DataFrame:
        try:
            r = self._minio.get_object(bucket_name=CANDIDATES_BUCKET, object_name=f"{mart}/{fname_date}.parquet")
            data = r.read()
            r.close()
            r.release_conn()
        except S3Error as e:
            raise FileNotFoundError(f"No candidate file: s3://{CANDIDATES_BUCKET}/{mart}/{fname_date}.parquet") from e
