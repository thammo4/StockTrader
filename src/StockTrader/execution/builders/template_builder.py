			...
			orders.append(
				MLegOrderRequest(
					...
					order_type = MLegOrderType(
						self._resolve(
							self.template["order_type"], row
						)
					),
					symbol = str(self._resolve(self.template["symbol"], row)) if "symbol" in self.template else legs[0].symbol,
					limit_price = self._resolve_optional_float(
						self.template.get("limit_price"), row),
