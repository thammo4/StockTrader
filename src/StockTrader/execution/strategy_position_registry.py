...
class StrategyPositionRegistry:
     ...
     def _validate_policy(
          ...
          #
          # Threshold 1.0 is excluded: requires a zero close price, which can
          # never pass close-price validation.
          #
     ...
     #
     # On update, entry_order_id and enabled are preserved unless explicitly
     # provided. Re-registering to change policy does not re-enable a disabled
     # position or drop its entry linkage.
     #
     ...
     def register(
          ...
          #
          # Preserve Entry Linkage + Enabled State Unless Explicitly Provided
          #

          if (
               entry_order_id is None
               and existing is not None
               and pd.notna(existing.get("entry_order_id"))
          ):
               entry_order_id = int(existing["entry_order_id"])

          if enabled is None:
               if existing is not None and pd.notna(existing.get("enabled")):
