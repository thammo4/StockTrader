...
class TraderAdapter:
    ...
    def cancel_order(self, cancel: CancelRequest) -> dict:
        if cancel.asset_type == AssetType.OPTION:
            ...
            if self._equities is None:
                raise RunTimeError("TraderAdapter got equity cancel without equities client")
    ...
    def place_order(self, order: OrderRequestUnion) -> dict:
        if isinstance(order, MLegOrderRequest):
            ...
            if self._equities is None:
                raise RuntimeError("TraderAdapter received equity order without equities client")
...
class SimpleOrderExecutor(OrderExecutor):
    ...
    def _cancel_one(self, cancel: CancelRequest) -> CancelResult:
        try:
            ...
            return CancelResult(
                request=cancel, success=status == "ok", cancel_ts=datetime.now(timezone.utc).isoformat()
            )
        except Exception as e:
            logger.error(f"Cacnel failed: order_id={cancel.order_id}: {str(e)}")
            return CancelResult(
                request=cancel, success=False, error=str(e), cancel_ts=datetime.now(timezone.utc).isoformat()
            )
    ...
    def _execute_one(self, order: OrderRequestUnion) -> ExecutionResult:
        try:
            ...
            return ExecutionResult(
                request=order, success=False, error=str(e), submit_ts=datetime.now(timezone.utc).isoformat()
            )

    def _dry_run_result(self, order: OrderRequestUnion) -> ExecutionResult:
        logger.info(f"DRY_RUN: {order}")
        return ExecutionResult(
            request=order, success=True, order_id=-1, submit_ts=datetime.now(timezone.utc).isoformat()
        )

    def _dry_run_cancel(self, cancel: CancelRequest) -> CancelResult:
        logger.info(f"DRY_RUN cancel: order_id={cancel.order_id}")
        return CancelResult(request=cancel, success=True, cancel_ts=datetime.now(timezone.utc).isoformat())
