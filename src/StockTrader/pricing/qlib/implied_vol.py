...
from StockTrader.pricing.errors import InputValidationError, IVBracketError, IVConvergeError, QLibError
...
def bracket_root(
    f_price: Callable[[float], float],
    price_target: float,
    *,
    cfg: BrentIVConfig,
    occ: Optional[str] = None,
    σ_guess: Optional[float] = None,
...
def solve_implied_vol_brent(
    f_price: Callable[[float], float],
    price_target: float,
    *,
    cfg: Optional[BrentIVConfig] = None,
    occ: Optional[str] = None,
    σ_guess: Optional[float] = None,
) -> float:
    # _require_scipy()
    ...
    if not _is_finite(price_target) or float(price_target) < 0.0:
        raise InputValidationError(
            message="Invalid price_target for implied vol solver", occ=occ, details={"price_target": price_target}
        )

    σ_lower, σ_upper, f_lower, f_upper = bracket_root(f_price, float(price_target), cfg=cfg, occ=occ, σ_guess=σ_guess)
...
def try_solve_implied_vol_brent(
    f_price: Callable[[float], float],
    price_target: float,
    *,
    cfg: Optional[BrentIVConfig] = None,
    ...
    try:
        σ = solve_implied_vol_brent(f_price, price_target, cfg=cfg, occ=occ, σ_guess=σ_guess)
