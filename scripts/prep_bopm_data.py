    ...
    try:
        ...
        def construct_div_periods(div_df):
            ...
            try:
                ...
                div_df["start_date"] = np.where(
                    div_df["prev_ex_date"].isna(), div_df["ex_date"] - pd.DateOffset(months=3), div_df["prev_ex_date"]
                )
        ...
        def merge_options_dividends(options_df, div_periods):
            ...
            try:
                ...
                criteria_2 = df_xjoin["expiration"].between(
                    df_xjoin["start_date"], df_xjoin["end_date"], inclusive="left"
                )
                criteria_3 = (df_xjoin["date"] <= df_xjoin["start_date"]) & (
                    df_xjoin["end_date"] <= df_xjoin["expiration"]
                )
                ...
                df_grouped = (
                    df_filtered.groupby(["date", "expiration", "ttm", "midprice", "strike", "call_put", "act_symbol"])
                    .agg(div_amt=("div_weighted", "sum"), div_count=("ex_date", "count"))
                    .reset_index()
                )
