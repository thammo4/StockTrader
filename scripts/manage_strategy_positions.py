...
#              --price-point mid
#
#         (re-registering an existing occ updates policy; enabled state and
#          entry_order_id are preserved unless --entry-order-id is provided)
#
...
def manage_strategy_positions(
     ...
     if command == "register":
          ...
          if price_point not in PRICE_POINTS:
               raise ValueError(f"price_point={price_point} not in {sorted(PRICE_POINTS)} [manage_strategy_positions]")
...
def main():
     ...
     p_register = subparsers.add_parser("register", help="Register or update an occ position for auto strategy management")
     ...
     p_register.add_argument("--price-point", choices=sorted(PRICE_POINTS), default="mid")
