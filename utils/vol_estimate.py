...
def vol_estimate(symbol, weeks=2.25, window_days=9, is_live=False):
    """
    ...
    σ_hat = (
        np.log(df["close"]).diff().rolling(window=9).std().iloc[-1]
    )  # index -1 will be the most recent trading day's volatility
